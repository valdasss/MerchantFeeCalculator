...
namespace MerchantFeeCalculator.Domain.Models
{
    public class MerchantFactory
    {
        ...
        public Merchant CreateMerchant(string merchantName, Transaction transaction, decimal transactionCommision)
        {
            if (IsMerchantStandart(merchantName))
                return new StandartMerchant(merchantName, transaction, transactionCommision);
            else
                return new BigMerchant(merchantName, transaction, transactionCommision, _bigMerchantDiscount[merchantName]);
        }
